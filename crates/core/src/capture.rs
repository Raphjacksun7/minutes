...
impl VoiceCaptureStream {
    fn start(plan: &DualCapturePlan) -> Result<Self, CaptureError> {
        #[cfg(all(feature = "pocketstation-capture", target_os = "macos"))]
    ...
    fn start_fallback(plan: &DualCapturePlan) -> Result<Self, CaptureError> {
        #[cfg(all(feature = "pocketstation-capture", target_os = "macos"))]
    ...
    fn receiver(&self) -> crossbeam_channel::Receiver<crate::streaming::AudioChunk> {
        match self {
            Self::Cpal(stream) => stream.receiver.clone(),
            #[cfg(all(feature = "pocketstation-capture", target_os = "macos"))]
            Self::PocketStation(stream) => stream.receiver.clone(),
        }
    }
    ...
    fn recovery_confirmed(&self) -> bool {
        match self {
            Self::Cpal(stream) => !stream.has_error(),
            #[cfg(all(feature = "pocketstation-capture", target_os = "macos"))]
    ...
    fn independent_failure(&self) -> bool {
        match self {
            Self::Cpal(_) => false,
            #[cfg(all(feature = "pocketstation-capture", target_os = "macos"))]
    ...
    fn delivering_frames(&self) -> bool {
        match self {
            Self::Cpal(_) => true,
            #[cfg(all(feature = "pocketstation-capture", target_os = "macos"))]
            Self::PocketStation(stream) => {
    ...
    fn report_recovery_context(&self) {
        #[cfg(all(feature = "pocketstation-capture", target_os = "macos"))]
    ...
    fn recover(
        ...
        match self {
            ...
            Self::PocketStation(mut stream) => {
                ...
                tracing::info!(
                    source_id = replacement.source_id.get(),
                    source_generation = replacement.source_generation,
                    discontinuity_epoch = replacement.discontinuity_epoch,
                    action = ?action,
                    "PocketStation microphone recovery attached a new source generation"
                );
                Ok(RecoveredVoiceStream {
                    stream: Self::PocketStation(stream),
                    lineage_floor: Some(VoiceLineageFloor {
                        source_generation: replacement.source_generation,
                        discontinuity_epoch: replacement.discontinuity_epoch,
                    }),
                })
...
impl DualCaptureWriters {
    fn new(output_path: &Path) -> Result<Self, CaptureError> {
        ...
        Ok(Self {
            ...
            voice_lineage_path: voice_lineage_path_for(output_path).ok_or_else(|| {
                CaptureError::Io(std::io::Error::other(
    ...
    loop {
        ...
        if voice_issue {
            ...
            if let Some(stream) = voice_stream.as_ref() {
                stream.report_recovery_context();
            }
            ...
            while let Ok(chunk) = system_backend_rx.try_recv() {
                queue_system_chunk(
                    chunk,
                    system_slot_base,
                    &mut next_slot,
        ...
        if system_issue {
            ...
            system_stream.take();
            while system_backend_rx.try_recv().is_ok() {}
