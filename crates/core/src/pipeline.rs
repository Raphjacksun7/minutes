...
/// plus `.voice.wav` and `.system.wav` PCM stems beside it. Decoding the `.mov`
/// for transcription produces audio at 2x real duration, so whisper receives
/// garbled samples and emits gibberish. A stopped or crashed capture can leave
/// valid payload behind placeholder RIFF/data lengths; verified recoverable
/// headers are repaired before the shared diarization classifier runs. When
/// both stems are usable, this helper mixes them into a 16kHz mono PCM via
/// `ffmpeg amix` and returns a prepared input to the transcriber. When
...
fn prepare_transcription_input_with_format(
    ...
    repair_recoverable_native_call_stem_headers(&canonical)?;
...
fn repair_recoverable_native_call_stem_headers(canonical_mov: &Path) -> Result<(), MinutesError> {
    let Some(parent) = canonical_mov.parent() else {
        return Ok(());
    };
    let Some(stem_name) = canonical_mov.file_stem().and_then(|stem| stem.to_str()) else {
        return Ok(());
    };

    for (source, path) in [
        ("microphone", parent.join(format!("{stem_name}.voice.wav"))),
        ("system", parent.join(format!("{stem_name}.system.wav"))),
    ] {
        let probe = crate::stem_probe::probe_and_repair(&path).map_err(|reason| {
            crate::error::TranscribeError::NativeCaptureStemMixUnavailable {
                reason: format!(
                    "Minutes could not safely probe or repair {source} stem {}: {reason}",
                    path.display()
                ),
            }
        })?;
        if probe.repaired {
            tracing::warn!(
                source,
                stem = %path.display(),
                format_repaired = probe.format_repaired,
                usable = probe.is_usable(),
                stem_state = %probe.description(),
                "repaired recoverable native-call WAV header before transcription"
            );
        }
    }
    Ok(())
...
mod prepare_transcription_input_tests {
    ...
    fn write_unfinalized_f32_wav(path: &std::path::Path, audible: bool) {
        ...
        bytes[64..68].copy_from_slice(&192_000_u32.to_le_bytes());
        bytes[68..70].copy_from_slice(&4_u16.to_le_bytes());
        bytes[70..72].copy_from_slice(&32_u16.to_le_bytes());
        ...
        bytes[4_092..4_096].copy_from_slice(&0_u32.to_le_bytes());
        for sample in 0..48_000 {
            let value = if audible {
                (2.0 * std::f32::consts::PI * 440.0 * sample as f32 / 48_000.0).sin() * 0.25
            } else {
                0.0
            };
    ...
    #[test]
    fn repairs_unfinalized_voice_header_before_selecting_survivor() {
        let dir = tempfile::TempDir::new().unwrap();
        let mov = dir.path().join("repair-single.mov");
        let voice = dir.path().join("repair-single.voice.wav");
        let system = dir.path().join("repair-single.system.wav");
        fs::write(&mov, b"x").unwrap();
        write_unfinalized_f32_wav(&voice, true);
        fs::write(&system, b"").unwrap();

        let result = prepare_transcription_input(&mov).expect("repaired voice stem is recoverable");
        let PreparedTranscriptionInput::SingleStem { path, .. } = result else {
            panic!("repaired voice stem must be selected without decoding the mov")
        };
        assert_eq!(path, voice.canonicalize().unwrap());

        let bytes = fs::read(&voice).unwrap();
        assert_eq!(
            u32::from_le_bytes(bytes[4..8].try_into().unwrap()),
            (bytes.len() - 8) as u32
        );
        assert_eq!(
            u32::from_le_bytes(bytes[0xffc..0x1000].try_into().unwrap()),
            (bytes.len() - 0x1000) as u32
        );
    }
